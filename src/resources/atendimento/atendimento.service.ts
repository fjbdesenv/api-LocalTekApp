...
export class AtendimentoService {
  ...
  async update(codigo: number, updateAtendimentoDto: UpdateAtendimentoDto): Promise<Atendimento> {
     
    try {
      this.atendimento = await this.atendimentoRepository.findOneBy({ codigo });
      
      if(this.atendimento){
        this.atendimento = Object.assign(this.atendimento, updateAtendimentoDto);
        
        await this.atendimentoRepository.update({codigo}, updateAtendimentoDto);
      }
    } catch (error) {
      this.error.erro500(error.message);
    }
  ...
  async remove(codigo: number): Promise<string> {
    try {
      ...
      result.affected > 0 ? this.atendimento = new Atendimento() : this.atendimento = undefined;
